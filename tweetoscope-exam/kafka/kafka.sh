...
case $ACTION in 
	...
	run_command "Starting the zookeeper server" "sudo $KAFKA_PATH/bin/zookeeper-server-start.sh -daemon $ZOOKEEPER_CONFIG" "sleep 2"

	run_command "Starting the kafka server" "sudo $KAFKA_PATH/bin/kafka-server-start.sh -daemon $SERVER_CONFIG" "sleep 2"
	;;
    stop)
	# Stop the kafka server
	run_command "Stopping the kafka server" "sudo $KAFKA_PATH/bin/kafka-server-stop.sh" "sleep 2"

	# Stop the zookeeper daemon
	run_command "Stopping the zookeeper server" "sudo $KAFKA_PATH/bin/zookeeper-server-stop.sh"
	;;
    *)
